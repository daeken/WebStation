...
function decompile(pc, inst, emit) {
	switch((inst) >>> (0x1a)) {
		case 0x0: {
			switch((inst) & (0x3f)) {
				case 0x0: {
				...
				case 0x8: {
					/* JR */
					$rs = ((inst) >>> (0x15)) & (0x1f);
					emit("state.branch(((state.regs)[" + $rs + "]) >>> (0x0));");
					return(true);
					break;
				}
				case 0x9: {
		...
		case 0x1: {
			switch(((inst) >>> (0x10)) & (0x1f)) {
				case 0x0: {
					...
					emit("if((((state.regs)[" + $rs + "]) | (0x0)) < (0x0)) { state.branch(" + $target + ") }");
					return(true);
					break;
				}
				case 0x1: {
		...
		case 0x4: {
			...
			$imm = (inst) & (0xffff);
			$target = ((pc) + (0x4)) + (signext(0x12, (($imm) << (0x2)) >>> (0x0)));
			emit("if((((state.regs)[" + $rs + "]) >>> (0x0)) == (((state.regs)[" + $rt + "]) >>> (0x0))) { state.branch(" + $target + ") }");
			return(true);
			break;
		}
		case 0x5: {
			...
			$imm = (inst) & (0xffff);
			$target = ((pc) + (0x4)) + (signext(0x12, (($imm) << (0x2)) >>> (0x0)));
			emit("if(((state.regs)[" + $rs + "]) != ((state.regs)[" + $rt + "])) { state.branch(" + $target + ") }");
			return(true);
			break;
		}
		case 0x6: {
		...
		case 0xf: {
			...
			$rt = ((inst) >>> (0x10)) & (0x1f);
			$imm = (inst) & (0xffff);
			if(($rt) != (0x0)) { emit("(state.regs)[" + $rt + "] = (((" + $imm + ") << (0x10)) >>> (0x0)) >>> (0x0);"); }
			return(true);
			break;
		}
		case 0x10: {
